...
class captureLogs {
	...
	getLogs() {
		try {
			...
			console.log(this.formatDate(new Date()) + ' [err] error parsing logfile: ', error.message);
			return [];
		}
	}
	...
	saveLogs() {
		try {
			...
			console.log(this.formatDate(new Date()) + ' [err] error writing logfile: ', error.message);
			return false;
		}
	}
	...
	deleteLogs() {
		try {
			...
			console.log(this.formatDate(new Date()) + ' [err] error deleting logfile: ', error.message);
			return false;
		}
	}
	...
	captureStdOut() {
		...
		this.captureStdout.capture((string) => {
			if (string.indexOf('[log]') == -1) {
				string = this.formatDate(new Date()) + ' [log] [Libraries] ' + string
			}
	...
	captureStdErr() {
		...
		this.captureStderr.capture((string) => {
			if (string.indexOf('[err]') == -1) {
				string = this.formatDate(new Date()) + ' [err] [Libraries] ' + string
			}
			if (this.logArray.length >= this.logLength) {
				this.logArray.shift();
			}
	...
	formatDate(date) {
		return (new Date(new Date().getTime() + (new Date().getTimezoneOffset() * 60 * 1000)).toLocaleString('en-US', { day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: this.homey.clock.getTimezone(), hour12: false }).replace(',', ''));
	}
